				...
				if($check_duty_rest == 1) {
					$premium_pay = $this->salary * 0.3;
						...
						if(date("H:i",strtotime($time_duty[0]->time)) > date("H:i",strtotime($time_in)) ) {				
							...
							switch ($date_status) {
								case 0:
									array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => (($this->total_cola_salary + $this->salary + $premium_pay) - ($total_deduct_late + $total_deduct_undertime)), "premium_pay" => $premium_pay, "late_duration" => $late_diff, "undertime_duration" => $undertime_diff ,"cola_duration" => $this->cola_duration, "date" => $perdate->time]);
									break;						
								case 1:
						...
						//kung undertime sya
							...
							switch ($date_status) {
								case 0:
									array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => (($this->total_cola_salary + $this->salary + $premium_pay) - ($total_deduct_late + $total_deduct_undertime)), "premium_pay" => $premium_pay, "late_duration" => 0, "undertime_duration" => $undertime_diff ,"cola_duration" => $this->cola_duration, "date" => $perdate->time]);
									break;						
								case 1:
									$this->salary = ($this->salary * 0.3) + $this->salary;
									array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => (($this->total_cola_salary + $this->salary + $premium_pay) - ($total_deduct_late + $total_deduct_undertime)), "premium_pay" => $premium_pay, "late_duration" => 0, "undertime_duration" => $undertime_diff ,"cola_duration" => $this->cola_duration, "date" => $perdate->time]);
						...
						//kung wlay late
							...
							switch ($date_status) {
								case 0:
									array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => ($this->total_cola_salary + $this->salary + $premium_pay), "premium_pay" => $premium_pay, "late_duration" => 0, "undertime_duration" => 0, "cola_duration" => $this->cola_duration, "date" => $perdate->time]);
									break;						
								case 1:
									$this->salary = ($this->salary * 0.3) + $this->salary;
									array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => ($this->total_cola_salary + $this->salary + $premium_pay), "premium_pay" => $premium_pay, "late_duration" => 0, "undertime_duration" => 0, "cola_duration" => $this->cola_duration, "date" => $perdate->time]);
					...
					if(date("H:i",strtotime($time_duty[0]->time)) > date("H:i",strtotime($time_in)) ) {				
						...
						switch ($date_status) {
							case 0:
								array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => (($this->total_cola_salary + $this->salary) - ($total_deduct_late + $total_deduct_undertime)), "premium_pay" => 0, "late_duration" => $late_diff, "undertime_duration" => $undertime_diff ,"cola_duration" => $this->cola_duration, "date" => $perdate->time]);
								break;						
							case 1:
								$this->salary = ($this->salary * 0.3) + $this->salary;
								array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => (($this->total_cola_salary + $this->salary) - ($total_deduct_late + $total_deduct_undertime)), "premium_pay" => 0, "late_duration" => $late_diff, "undertime_duration" => $undertime_diff ,"cola_duration" => $this->cola_duration, "date" => $perdate->time]);
					...
					//kung undertime sya
						...
						switch ($date_status) {
							case 0:
								array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => (($this->total_cola_salary + $this->salary) - ($total_deduct_late + $total_deduct_undertime)), "premium_pay" => 0, "late_duration" => 0, "undertime_duration" => $undertime_diff ,"cola_duration" => $this->cola_duration, "date" => $perdate->time]);
								break;						
							case 1:
								$this->salary = ($this->salary * 0.3) + $this->salary;
								array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => (($this->total_cola_salary + $this->salary) - ($total_deduct_late + $total_deduct_undertime)), "premium_pay" => 0, "late_duration" => 0, "undertime_duration" => $undertime_diff ,"cola_duration" => $this->cola_duration, "date" => $perdate->time]);
					...
					//kung wlay late
						...
						switch ($date_status) {
							case 0:
								array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => ($this->total_cola_salary + $this->salary), "premium_pay" => 0, "late_duration" => 0, "undertime_duration" => 0, "cola_duration" => $this->cola_duration, "date" => $perdate->time]);
								break;						
							case 1:
								$this->salary = ($this->salary * 0.3) + $this->salary;
								array_push($this->data_array,["employee_id" => $id, "calculate_id" => $calculate, "salary" => ($this->total_cola_salary + $this->salary), "premium_pay" => 0, "late_duration" => 0, "undertime_duration" => 0, "cola_duration" => $this->cola_duration, "date" => $perdate->time]);
			...
			switch ($salary_status) {
				case 0:										
					...
					foreach ($result as $emp) {
			            ...
			            $row[] = $emp['employee_id'];
			            $row[] = $emp['name'];
			            $row[] = (double) number_format($emp['salary'],2, '.','');
		...
		if (admin_login()) {
			...
			if(!$getCalculated) {
				...
				foreach ($getCalculated as $emp) {										
			            ...
			            $row[] = $name;
			            $row[] = $emp->salary;
			            $row[] = '<button onclick="show_details('."'".$emp->employee_id."'".');" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="right" title="View Details" id="btnR2"><i class="fa fa-file"></i></i></button>';	            	           
			            $data[] = $row;
			        }
